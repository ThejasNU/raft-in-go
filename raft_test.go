...
func Test2(t *testing.T) {
	...
	// ReceiveClientCommand 6 to original leader, even though it's disconnected. Should not reflect.
	cluster.SubmitClientCommand(origLeaderId, "Set X = X-5")

	newLeaderId := cluster.getClusterLeader()

	// ReceiveClientCommand 7.. to new leader.
	...
	// ReceiveClientCommand 8 and check it's fully committed.
